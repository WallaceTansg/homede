...
matchscore <- function(Ptitle,Pdesc,Sterm){
   for(i in 1:length(Sterm)){
     ScoreMG<-pairwiseAlignment(pattern = c(Ptitle[i],Pdesc[i]), subject = Sterm[i],
                                type = "global", gapOpening = 0, gapExtension = 1)
     ScoreMG2<-pairwiseAlignment(pattern = c(Ptitle[i]), subject = Pdesc[i],
                                type = "global", gapOpening = 0, gapExtension = 1)
     slotNames(ScoreMG)
     ScoreMGS<-ScoreMG@score
     slotNames(ScoreMG2)
     ScoreMGS2<-ScoreMG2@score
  }
  return(c(ScoreMGS,ScoreMGS2))
}

#http://ntz-develop.blogspot.tw/2011/03/fuzzy-string-search.html

cat("Get match score of each pair in train dataset")
mscore<-as.data.frame(t(mapply(matchscore,train$product_title,train$product_description,train$search_term)))
