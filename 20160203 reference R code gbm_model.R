...
matchtitlterm <- function(Ptitle,Sterm){
  for(i in 1:length(Sterm)){
    Scoretitle<-pairwiseAlignment(pattern = c(as.character(Ptitle[i])), subject = as.character(Sterm[i]),
                               type = "local", gapOpening = 1, gapExtension = 0)
    
    slotNames(Scoretitle)
    Scoretitle<-Scoretitle@score
  }
  return(Scoretitle)
}
...
matchdesc <- function(Pdesc,Sterm){
  for(i in 1:length(Sterm)){
    Scoredesc<-pairwiseAlignment(pattern = c(as.character(Pdesc[i])), subject = as.character(Sterm[i]),
                                  type = "local", gapOpening = 1, gapExtension = 0)
    
    slotNames(Scoredesc)
    Scoredesc<-Scoredesc@score
  }
  return(Scoredesc)
}
...
matchdesctitle <- function(Pdesc,Ptitle){
  for(i in 1:length(Ptitle)){
    Scoredesctitle<-pairwiseAlignment(pattern = c(as.character(Pdesc[i])), subject = as.character(Ptitle[i]),
                                 type = "local", gapOpening = 1, gapExtension = 0)
    
    slotNames(Scoredesctitle)
    Scoredesctitle<-Scoredesctitle@score
  }
  return(Scoredesctitle)
}
...
test_mscoretitle<-as.data.frame(mapply(matchtitlterm,train$product_title,train$search_term))
test_mscoredesc<-as.data.frame(mapply(matchdesc,train$product_description,train$search_term))
test_mscoredesctitle<-as.data.frame(mapply(matchdesctitle,train$product_description,train$product_title))
