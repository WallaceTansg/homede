...
test$revelence<-testS(test$mscore_termtitel,test$mscore_termdesc,test$mscore_titeldesc)


cat("A simple linear model on number of words and number of words that match\n")
sample(train, size=, replace=T)
library(kknn)
...
TR <- ifelse(test$revelence<=1.9,1,TR)
TR <- ifelse(test$relevance>2.33,3,TR)
TR <- ifelse(test$relevance>=1.9 & test$relevance<=2.33,2,TR)
TR

table(TR)

testid<-data.frame(test$id,TR)

submission <- data.frame(id=test_mscore$id,relevance=test_relevance)
